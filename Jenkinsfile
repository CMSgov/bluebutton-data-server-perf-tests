...
stage('Test the Staging ENV') {
	if (params.test_staging) {
		milestone(label: 'stage_test_staging_start')

		node {
			insideAnsibleContainer {
				sh "cd ansible && ansible-playbook fhir-stress-test-temp-testers.yml -e \"target_env=staging num_servers=${params.env_num_servers}\" --connection=paramiko"
			}
		}
	} else {
		org.jenkinsci.plugins.pipeline.modeldefinition.Utils.markStageSkippedForConditional('Test the Staging ENV')
	}
}
